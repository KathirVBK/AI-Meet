...
class ActionItem(BaseModel):
    ...
    owner: str = Field(description="Name of the person responsible for the task")
    ...
    evidence: str = Field(default="Not provided", description="Transcript snippet or reasoning as evidence")
...
class Decision(BaseModel):
    """Represents a structured decision made during a meeting."""
    decision: str = Field(description="The core decision that was made")
    status: str = Field(description="Status of the decision, e.g., Confirmed, Proposed, Rejected")
    evidence: Optional[str] = Field(default=None, description="Transcript snippet or reasoning as evidence")
...
class MeetingMinutes(BaseModel):
    ...
    agenda: List[str] = Field(description="List of agenda items discussed")
    discussion_points: List[str] = Field(description="Key discussion points from the meeting")
    decisions: List[Decision] = Field(default_factory=list, description="List of decisions made during the meeting")
    action_items: List[ActionItem] = Field(default_factory=list, description="List of action items with owners and deadlines")
...
class GraphState(BaseModel):
    ...
    analysis: dict = Field(default_factory=dict, description="Structured meeting analysis from the analyzer agent")
